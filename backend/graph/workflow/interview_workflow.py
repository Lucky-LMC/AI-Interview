...
from backend.graph.nodes import (
    ...
    search_resources_node,   # 搜索学习资源节点
...
def create_interview_graph():
    ...
    workflow.add_node("search_resources", search_resources_node)     # 搜索学习资源节点（Coach Agent）
    ...
    workflow.add_conditional_edges(
        ...
        lambda state: "finish" if state['is_finished'] else "continue",
        {
            ...
            "finish": "search_resources"        # 已完成 -> 搜索学习资源（Coach Agent）
        }
    )

    # 搜索资源 -> 生成报告
    workflow.add_edge("search_resources", "generate_report")
